...
Note that~$\mathcal{B}_{\varphi}$ has the structure as defined in Def.~\ref{ch04:def:nba},
...
Def.~\ref{ch04:def:decomposable-transition} below.
...
but via another intermediate state, as illustrated in Fig.~\ref{ch04:fig:example_decomposable}.
...
\begin{definition}[Decomposable Transition]\label{ch04:def:decomposable-transition}
...
An example of decomposable transitions is shown in Fig.~\ref{ch04:fig:example_decomposable}.
...
\begin{proof}
	...
	$q_k=\delta(q_i,\sigma_{ik}),q_j=\delta(q_k,\sigma_{kj})$,
        $\sigma_{ik}\cup \sigma_{kj}=\sigma_n$ based on Def.~\ref{ch04:def:decomposable-transition}.
	...
	According to Def.~\ref{ch04:def:decomposable-transition}, this process will terminate
...
decomposable satisfied by Def.~\ref{ch04:def:decomposable-transition}.}
\label{ch04:fig:example_decomposable}
\end{figure}
%========================================

\begin{example}\label{ch04:eq:prune-nba}
The NBA~$\mathcal{B}_{\varphi}$ associated with the task formula in~\eqref{ch04:example:task}
...
%==============================
\subsection{Task Decomposition}\label{ch04:subsubsec:task-decompose}
...
\begin{definition}[Decomposition and Subtasks] \label{ch04:def:subtasks}
...
\begin{equation}\label{ch04:eq:subtask}
...
\begin{example}
	\label{ch04:example:subtask}
        As shown in Fig.~\ref{ch04:fig:example_decomposable}~,
...
\begin{definition}[Partial Relations]\label{ch04:def:partial}
...
%==========
\begin{remark}\label{ch04:remark:duration}
Note that most related work in~\cite{kantaros2020stylus, guo2015multi,
...
see e.g., \cite{luo2021temporal}.}
On the contrary, as described in Sec.~\ref{ch04:subsec:multi-agent},
...
The above definition is illustrated in Fig.~\ref{ch04:fig:partial}.
...
%==========
\begin{remark}\label{ch04:remark:partial-order-motivation}
The above two relations, i.e., $\preceq_\varphi$ and $\neq_\varphi$,
are chosen in the definition of R-posets due to following observations:
as illustrated in Fig.~\ref{ch04:fig:partial} and explained in Remark~\ref{ch04:remark:duration},
...
\{\omega_1,\omega_2\}\notin\neq_\varphi$.
These cases are summarized in Fig.\ref{ch04:fig:example_remark_4}.}
...
%==========
\begin{remark}\label{ch04:remark:compare-poset}
...
\begin{figure}
	...
	\label{ch04:fig:example_remark_4}
\end{figure}
%========================================
...
\begin{definition}[Language of R-poset]\label{ch04:def:language-poset}
...
\begin{equation}\label{ch04:eq:poset-language}
...
\begin{algorithm}[h]\footnotesize
	...
	\While{$time<t_0$}{
		\tcc{Subtask decomp. by Def.~\ref{ch04:def:subtasks}}
		Compute~ $\Omega$ and word~$w$ given~$\rho$;
		\label{ch04:alg-line:decompose}\\
		\If{$w \notin \mathcal{L}_\varphi$\label{ch04:alg1:inlp}}{
			Set~$P=(\Omega,\, \preceq_{\varphi}=\mathbb{F},\neq_{\varphi}=2^{\Omega})$,
			and $\preceq_{\varphi}=\mathbb{F}$;\label{ch04:alg-line:initial}\\
			Set~$L(P)=\{w\},{Que=[w]}$, $I_1=I_2=\emptyset$;\label{ch04:alg-line:que}\\
			\tcc{Reduce partial relations}
			\While{$|Que|>0$ and $time>t_0$ \label{ch04:alg1:begin_que}}{
				$w \leftarrow Que.pop()$;\\
				\For{$i=1,2,\cdots,|w|-1$}{
					$\omega_{1}=w[i]$, $\omega_{2}=w[i+1]$;\\
					$w' \leftarrow $ Switching~$\omega_{1}$
					and~$\omega_{2}$ within~$w$\label{ch04:alg1:switch};\\
					\eIf{$w'$ is accepting\label{ch04:alg1:waccepting}}{

						Add $(\omega_{1},\,\omega_{2})$ to $I_1$;\\
						Add $w'$ to $Que, L(P)$ if not in $L(P)$;\label{ch04:alg1:add-w}
						%Add $W'$ to $L(P)$ if not in $L(P)$;
					}{Add $(\omega_{1},\,\omega_{2})$ to $I_2$;\label{ch04:alg1:end_que}}
				}
				Remove $\{I_1\,\backslash\, I_2\}$
				from~$\preceq_{\varphi} $,{update~$time$};\label{ch04:alg-line:remove}\\
				{\textbf{If} $time>t_0$, \textbf{return}  $\mathcal{P}_{\varphi},\mathcal{L}_{\varphi}$.}\\}
				\For{$\boldsymbol{\hat{\omega}} \subseteq \neq_{\varphi}$\label{ch04:alg-line:calculate_neq}}{
				$w' \leftarrow $ Replace~$\omega_i\in\boldsymbol{\hat{\omega}} $ in~$w$
				by~$\bigcup_{\omega_i\in\boldsymbol{\hat{\omega}}}\omega_i$;\label{ch04:alg1:word-neq}\\
				\If{$w'$ is accepting\label{ch04:alg1:waccepting-neq}}{
					Remove~$\boldsymbol{\hat{\omega}} $
					from~$\neq_{\varphi}$;\label{ch04:alg-line:remove_neq}\\
			}
			}
			\tcc{Self loop calculation }\For{$w$ in $L(P)$}{
				Get path $\rho'$ associated with $\sigma_\ell$ of $w'$;\\
				\For{$i=0,1,\cdots,|\rho'|$\label{ch04:alg-line:self-loop}}{
...
As summarized in Alg.~\ref{ch04:alg:compute-poset},
...
derived by following Definition~\ref{ch04:def:subtasks}~, see Line~\ref{ch04:alg-line:decompose}~.
Then, a R-poset $P$ is initialized as $P=(\Omega,\, \mathbb{F},\, 2^{\Omega})$
in Line~\ref{ch04:alg-line:initial}~,
namely, a fully-ordered R-poset as described after Definition~\ref{ch04:def:poset}~.
...
in the circle of Line~\ref{ch04:alg1:begin_que}~-\ref{ch04:alg-line:remove}~.
If so, it means the relative ordering of this two adjacent subtasks can
potentially be relaxed or removed from~$\preceq_{\varphi}$ as in Line~\ref{ch04:alg-line:remove}~.
...
{The queue $Que$ is used to store the set of accepting words $w$ and the iteration ends when $|Que|=0$ in Line~\ref{ch04:alg1:begin_que},
...
in Line~\ref{ch04:alg1:word-neq}~.
If this new word is accepting, it means that this subtasks set do not belong to the
relation~$\neq_{\varphi}$ in Line~\ref{ch04:alg-line:remove_neq}~. After that, labels of self loop $\sigma^s_\ell$ is calculated
by checking all feasible word in line~\ref{ch04:alg-line:self-loop}~.
Note that the resulting~$P$ is only one of the R-posets and the associated language is
given by~$L(P)$ as defined in~Definition~\ref{ch04:def:language-poset}~.
...
\begin{example}
	Continuing from example~\ref{ch04:example:subtask}, we can get an R-poset with
	$\preceq_\varphi=\{(1,2),(1,3)\},\neq_{\varphi}=\{\{2,3\}\}$.

\end{example}

%==============================
\begin{lemma}\label{ch04:lemma:accepting-poset}
Any R-poset within $\mathcal{P}_{\varphi}$ obtained by Alg.~\ref{ch04:alg:compute-poset}
is accepting.
\end{lemma}
\begin{proof}
...
To begin with, as shown in Line~\ref{ch04:alg1:add-w}, any word~$w$ added to~$L(P)$
is accepting. %正确性
...
due to the iteration process of~$Que$ in Line~\ref{ch04:alg1:begin_que}-\ref{ch04:alg1:end_que},
...
Then, $(\omega_i,\omega_j)\notin \preceq_\varphi$ holds if $ i<j$ and $w$ cannot satisfy the ``less equal'' constraints otherwise.
 Similar to the bubble sorting algorithm \cite{astrachan2003bubble},
...
$(\omega_i,\omega_j)$ are kept in $\preceq_\varphi$ for all $i<j$ as in Line \ref{ch04:alg-line:remove}.
...
Line~\ref{ch04:alg1:begin_que}-\ref{ch04:alg1:end_que} and then
verified in Line~\ref{ch04:alg1:word-neq}, as~$w\in L(P)$. This completes the proof.
\end{proof}



Since Alg.~\ref{ch04:alg:compute-poset} is an anytime algorithm,
...
given enough time, Alg.~\ref{ch04:alg:compute-poset} can generate
...
The above analysis is summarized in lemma~\ref{ch04:lemma:complete-poset}.

%==============================
\begin{lemma}\label{ch04:lemma:complete-poset}
The outputs of Alg.~\ref{ch04:alg:compute-poset}
...
Since the set of subtasks within the R-poset is simply
 ...
 Then, as discussed in Lemma~\ref{ch04:lemma:accepting-poset},~$w\in L(P_1)$ implies
 ...
 However, since the~$Que$ in Alg.~\ref{ch04:alg:compute-poset} iterates through all
...
the underlying DFS search scheme in Alg.~\ref{ch04:alg:compute-poset} is guaranteed
...
\label{ch04:fig:poset_language}
%--------------------
\end{figure}
%========================================

Similar to the Hasse diagram in~\cite{simovici2008mathematical},
the following graph can be constructed given one R-poset~$P_{\varphi}$.

\begin{definition}[R-posets Graph]\label{ch04:def:poset-graph}
...
As shown in Fig.~\ref{ch04:fig:poset_language}~,~$\mathcal{G}_{P_\varphi}$ can be dis-connected with multiple root nodes.
