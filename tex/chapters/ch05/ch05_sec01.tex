...
temporal constraints are released online, as illustrated in
Fig.~\ref{ch05:fig:first}. The focus is on an online co-optimization problem in
...
For each robot $i \in \mathcal{N}$, a local plan is a timed sequence
...
time $t_i^\ell$. The collective plan is
...
limitations in signal propagation and environmental occlusion. As illustrated in
Fig.~\ref{ch05:fig:communication-constraints}, successful communication between
...
Accordingly, each local plan $\tau_i$ should be understood as containing both
task-execution decisions and communication decisions.
For a collective plan $J$ and a horizon $T > 0$, let
$N_T(J) \triangleq
...
node $\nu \in \mathcal{T}$ is associated with a partial collective plan
\[
J_\nu \triangleq
\bigl[
...
(IV) \emph{Bounding}.
Each child node undergoes iterative bound refinement. As illustrated in
Fig.~\ref{ch05:fig:lower-bound}, at iteration $k$, a task
...
robots. The resulting optimization problem is
\begin{equation}
\begin{aligned}
...
through the routine $\texttt{SelCom}$. As illustrated in
Fig.~\ref{ch05:fig:comm-event}, this candidate moves the communication
...
executed online. As illustrated in Fig.~\ref{ch05:fig:framework}, robots
discover new tasks while moving in the workspace, and the detected task set is
