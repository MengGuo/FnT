...
little as possible.
A related approach by~\citet{kim2012approximate} constructs a relaxed
...
cost in \eqref{ch03:eq:prod-cost2} is given by:
\begin{equation}
...
\label{ch03:eq:cost}
\end{equation}
and the accumulated specification distance is defined as:
\begin{equation}
...
$\mathcal{A}^{\textup{soft}}$ is given by:
\begin{equation}
...
\citep{baier2008principles}. Denote by $r|_{Q_1}$ and $r|_{Q_2}$ the projections
...
W_r(\langle \pi_i,\, q_m \rangle,\,
\langle \pi_j,\, q_n \rangle)
\triangleq  w_c(\pi_i,\, \pi_j) + \alpha \cdot
...
transition as in~\citep{guo2013revising}, is often unnecessary. Instead, update
...
\citet{guo2013revising}. If no bridge exists, it calls
...
in~\citep{guo2014cooperative}. The hybrid controller then receives the updated
next target region.
...
\citep{chan2009shortest,misra2005dynamic}. Let $\Upsilon_i^t$ count accumulated
